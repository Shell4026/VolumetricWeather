...
void CSMPass::SetUsages(const FrameContext& frame)
{
	APass::SetUsages(frame);
	const VulkanImage* const shadowMapPtr = csm.GetShadowMap();
	if (shadowMapPtr == nullptr)
		return;
	AddUsage(shadowMapPtr->GetImage(), VkImageAspectFlagBits::VK_IMAGE_ASPECT_DEPTH_BIT, VkImageLayout::VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL);
}

void CSMPass::Record(const FrameContext& frame)
{
	...
	const float x = 0.f;
	const float y = 0.f;
	const float w = static_cast<float>(shadowMapPtr->GetInfo().extent.width);
	const float h = static_cast<float>(shadowMapPtr->GetInfo().extent.height);
	...
	for (uint32_t i = 0; i < csm.GetSliceCount(); ++i)
	{
		depthAttachment.imageView = shadowMapPtr->GetViews()[i];
		VkRenderingInfo renderingInfo{};
		renderingInfo.sType = VkStructureType::VK_STRUCTURE_TYPE_RENDERING_INFO;
		renderingInfo.pDepthAttachment = &depthAttachment;
		renderingInfo.renderArea = { { 0, 0 }, { shadowMapPtr->GetInfo().extent.width, shadowMapPtr->GetInfo().extent.height } };
		...
		for (const Drawable* drawable : drawables)
		{
			if (drawable->mesh == nullptr)
				continue;
			const glm::mat4 mvp = csm.GetViewProjMatrix(i) * drawable->modelMatrix;
			...
			vkCmdPushConstants(cmd, shader.GetPipelineLayout(), VkShaderStageFlagBits::VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4), &mvp);
...
void CSMPass::UpdateCSM(const Camera& mainCamera, const glm::vec3& lightDir)
{
	csm.Create(*ctx, mainCamera, lightDir, 4);
}

void CSMPass::PrepareResource(const VulkanContext& ctx, VkDescriptorSetLayout cameraSetLayout)
{
	const VkDevice device = ctx.GetDevice();
	...
	shader.Build(device, "shaders/csm.vert.spv", "", &pc);
}

void CSMPass::BuildPipeline(const VulkanContext& ctx)
{
	const VkDevice device = ctx.GetDevice();
